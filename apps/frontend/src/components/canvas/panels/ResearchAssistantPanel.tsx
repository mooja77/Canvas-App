...
export default function ResearchAssistantPanel({ onClose }: ResearchAssistantPanelProps) {
  ...
  const { messages, loading, indexing, indexed, error, loadHistory, sendMessage, indexCanvas, clearMessages } =
    useChatStore();
        ...
        {messages.length === 0 && !loading && (
          <div className="flex flex-col items-center justify-center h-full text-center px-4">
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400">
              Ask questions about your research data. The AI will search through your transcripts, codes, and memos to
              provide relevant answers.
            </p>
            <div className="mt-3 space-y-1">
              {[
                'What themes emerge across transcripts?',
                  ...
                  {msg.citations.map((c, i) => (
                    <div
                      ...
                      className={`rounded px-2 py-1 text-[10px] ${
                        msg.role === 'user' ? 'bg-purple-500/30' : 'bg-gray-200 dark:bg-gray-600'
                      }`}
                    >
