...
/**
 * Shown inside the Research Calendar modal when the user is on legacy
 * access-code auth. The calendar API (/api/calendar/events) requires an
 * email-linked account; legacy users previously saw an indefinite spinner
 * + a silent 401 (live QA finding #6). This explains the gate and links
 * to /account where they can link an email.
 */
function EmailAuthRequired() {
  return (
    <div className="flex flex-1 flex-col items-center justify-center gap-3 px-6 py-12 text-center">
      ...
      <p className="max-w-sm text-xs text-gray-500 dark:text-gray-400">
        Research Calendar syncs events to your account, which needs an email-linked login. You&rsquo;re currently signed
        in with an access code.
      </p>
      <Link
        to="/account"
        className="mt-1 inline-flex items-center gap-1.5 rounded-lg bg-blue-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-blue-700 transition-colors"
      >
...
export default function CalendarPanel({ onClose }: CalendarPanelProps) {
  ...
  const loadEvents = useCallback(async () => {
    // Legacy access-code auth can't reach /api/calendar/events — skip the
    // fetch entirely so there's no silent 401 + indefinite spinner.
    if (!emailAuthed) {
      setLoading(false);
      return;
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm" onClick={onClose}>
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="research-calendar-title"
      >
        {/* Header */}
        <div className="flex items-center justify-between border-b border-gray-200 dark:border-gray-700 px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <h2 id="research-calendar-title" className="text-lg font-semibold text-gray-900 dark:text-white">
              Research Calendar
            </h2>
          </div>
          <button
                      ...
                      <div className="space-y-2">
                        {upcomingEvents.map((evt) => {
                          ...
                          return (
                            <div
                              key={evt.id}
                              className={`rounded-lg ${colors.bg} p-3 border border-transparent hover:border-gray-200 dark:hover:border-gray-600 transition-colors`}
                            >
