...
export default function CanvasContextMenu({
  ...
  const Icon = ({ d, className }: { d: string; className?: string }) => (
    <svg
      className={className || 'h-3.5 w-3.5 text-gray-400'}
      fill="none"
      viewBox="0 0 24 24"
      strokeWidth={1.5}
      stroke="currentColor"
  ...
  // Place to the right of the (clamped) main menu. If near the right edge,
  // flip to the left.
  const submenuLeft = pos.x + 208 + 192 > window.innerWidth ? -192 : 208;
  ...
  return createPortal(
    <>
      ...
      >
        ...
        <div onMouseEnter={handleAnalysisEnter} onMouseLeave={handleAnalysisLeave}>
          <button className={`${btnClass} relative`} onClick={() => setShowAnalysis((prev) => !prev)}>
            <Icon
      ...
      {showAnalysis && (
        <div
          ...
          style={{
            left: pos.x + submenuLeft,
            top: pos.y + 100, // roughly aligned with the "Add Analysis..." button row
          }}
