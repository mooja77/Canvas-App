...
export default function ExcerptBrowserModal({ onClose }: ExcerptBrowserModalProps) {
  ...
  const filteredExcerpts = useMemo(() => {
    ...
    switch (sortMode) {
      case 'newest':
        items = [...items].sort(
          (a, b) => new Date(b.coding.createdAt).getTime() - new Date(a.coding.createdAt).getTime(),
        );
        break;
      case 'oldest':
        items = [...items].sort(
          (a, b) => new Date(a.coding.createdAt).getTime() - new Date(b.coding.createdAt).getTime(),
        );
        break;
      case 'source':
        items = [...items].sort((a, b) => a.transcriptTitle.localeCompare(b.transcriptTitle));
        break;
      case 'code':
        items = [...items].sort((a, b) => a.codeName.localeCompare(b.codeName));
        break;
      case 'length':
        items = [...items].sort((a, b) => b.coding.codedText.length - a.coding.codedText.length);
        break;
    }
    return items;
  ...
  const uniqueSources = useMemo(
    () => new Set(filteredExcerpts.map((e) => e.coding.transcriptId)).size,
    [filteredExcerpts],
  );
                            ...
                            <div className="divide-y divide-gray-50 dark:divide-gray-700/30">
                              {group.excerpts.map((e) => (
                                <ExcerptCard
                                  key={e.coding.id}
                                  excerpt={e}
                                  onCopy={handleCopyExcerpt}
                                  onDelete={handleDeleteExcerpt}
                                  onSelectCode={setSelectedQuestionId}
                                />
                              ))}
                            </div>
                          )}
              ...
              {!kwicWord.trim() ? (
                ...
                <table className="w-full text-xs">
                  ...
                  <tbody className="divide-y divide-gray-50 dark:divide-gray-700/30 font-mono">
                    {kwicResults.map((r, i) => (
                      <tr
                        key={`${r.transcriptId}-${r.offset}-${i}`}
                        className="hover:bg-blue-50/50 dark:hover:bg-blue-900/10"
                      >
                        <td
                          className="px-3 py-1.5 text-gray-500 dark:text-gray-400 truncate max-w-[100px]"
                          title={r.transcriptTitle}
                        >
                          {r.transcriptTitle}
                        </td>
                        <td className="px-1 py-1.5 text-right text-gray-500 dark:text-gray-400 truncate">{r.before}</td>
                        <td className="px-1 py-1.5 text-center font-semibold text-brand-600 dark:text-brand-400 bg-brand-50/50 dark:bg-brand-900/20">
                          {r.match}
                        </td>
