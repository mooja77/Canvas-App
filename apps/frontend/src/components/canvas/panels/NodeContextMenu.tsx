...
export default function NodeContextMenu({
  ...
  const coverageStats = useMemo(() => {
    ...
    const intervals = codings.map((c) => [c.startOffset, c.endOffset] as [number, number]).sort((a, b) => a[0] - b[0]);
    let codedChars = 0;
    let maxEnd = 0;
  ...
  const handleLinkCanvas = useCallback(
    (canvas: CodingCanvas) => {
  ...
  const btnClass =
    'flex w-full items-center gap-2.5 px-3 py-2 text-left text-xs text-gray-700 hover:bg-gray-50 dark:text-gray-300 dark:hover:bg-gray-750';
  const btnClassRed =
    'flex w-full items-center gap-2.5 px-3 py-2 text-left text-xs text-red-600 hover:bg-red-50 dark:text-red-400 dark:hover:bg-gray-750';
  ...
  const colorPickerSection = (
    <>
      <button onClick={() => setShowColorPicker((prev) => !prev)} className={btnClass}>
        <Icon d={icons.color} />
        Change Color
        <span className="ml-auto flex items-center gap-1.5">
          ...
          <Icon d={showColorPicker ? icons.chevronUp : icons.chevronDown} className="h-3 w-3 text-gray-400" />
        </span>
      </button>
      {showColorPicker && (
  ...
  const coverageSection = (
    <>
      <button onClick={() => setShowCoverage((prev) => !prev)} className={btnClass}>
        <Icon d={icons.coverage} />
        View Coverage
        <span className="ml-auto">
          <Icon d={showCoverage ? icons.chevronUp : icons.chevronDown} className="h-3 w-3 text-gray-400" />
        </span>
      </button>
  ...
  return createPortal(
    ...
    >
      ...
      <button onClick={handleOpenCanvasLinker} className={btnClass}>
        ...
        <span className="ml-auto">
          <Icon d={showCanvasLinker ? icons.chevronUp : icons.chevronDown} className="h-3 w-3 text-gray-400" />
        </span>
      </button>
      {existingRef && (
