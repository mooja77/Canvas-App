...
export default function CodingDetailPanel() {
  ...
  return (
    <div
      data-tour="canvas-detail-panel"
      className="animate-slide-in-right flex h-full w-full sm:w-80 shrink-0 flex-col border-l border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800"
    >
      {/* Header */}
        ...
        {codings.length === 0 ? (
            ...
            {filterTranscript !== 'all' ? (
              ...
              <>
                ...
                <p className="text-[10px] text-gray-300 dark:text-gray-600 mt-1">
                  Select text in a transcript and choose this code.
                </p>
              </>
            )}
          </div>
          ...
          <div className="space-y-2.5">
            {codings.map((coding: CanvasTextCoding) => (
              <div
                key={coding.id}
                className="rounded-xl border border-gray-100 bg-gray-50/80 p-3 dark:border-gray-700 dark:bg-gray-750/80 transition-all hover:shadow-sm"
              >
      ...
      <div className="border-t border-gray-200 px-4 py-2 dark:border-gray-700">
        <div className="flex items-center justify-between">
          ...
          {codings.length > 0 && (
            <span className="text-[10px] text-gray-400 tabular-nums">
              {codings
                .reduce((sum: number, c: CanvasTextCoding) => sum + (c.endOffset - c.startOffset), 0)
                .toLocaleString()}{' '}
              chars
            </span>
          )}
      ...
      {confirmDeleteId &&
        createPortal(
          <ConfirmDialog
            ...
            confirmLabel="Remove"
            onConfirm={async () => {
              await deleteCoding(confirmDeleteId);
