...
export default function SelectionToolbar({
  ...
  const btnClass =
    'rounded-md p-1.5 text-gray-500 hover:bg-gray-100 hover:text-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-gray-200 transition-colors';
  ...
  return (
    ...
    >
      <span className="px-1.5 text-[10px] font-medium text-gray-400">{selectedNodes.length} selected</span>
      ...
      {onAnalyzeSelection &&
        (() => {
          const transcriptIds = selectedNodes
            .filter((n) => n.type === 'transcript')
            .map((n) => n.id.replace('transcript-', ''));
          const questionIds = selectedNodes
            .filter((n) => n.type === 'question')
            .map((n) => n.id.replace('question-', ''));
      ...
      <button
        onClick={onDeleteAll}
        className="rounded-md p-1.5 text-red-400 hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/20 transition-colors"
