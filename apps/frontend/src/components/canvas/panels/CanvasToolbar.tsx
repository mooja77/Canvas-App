...
export default function CanvasToolbar({
  ...
  const [exportingDisclosure, setExportingDisclosure] = useState(false);
  ...
  const handleExportAiDisclosure = async () => {
    if (!activeCanvas) return;
    setExportingDisclosure(true);
    try {
      const res = await canvasApi.getAiDisclosure(activeCanvas.id);
      const markdown: string = res.data?.data?.markdown ?? '';
      const blob = new Blob([markdown], { type: 'text/markdown' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `${activeCanvas.name || 'canvas'}-ai-disclosure.md`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
      toast.success('AI disclosure downloaded');
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
    } catch (err: any) {
      toast.error(err.response?.data?.error || 'Failed to export AI disclosure');
    } finally {
      setExportingDisclosure(false);
    }
  };
          ...
          >
            ...
            >
              ...
              <DropdownItem
                ...
                label="Methods statement (for your paper)"
                onClick={() =>
                  requireAiConfig
                    ? requireAiConfig('Methods Statement', () => setShowMethodsStatement(true))
                    : setShowMethodsStatement(true)
                }
              />
              <DropdownItem
                icon={
