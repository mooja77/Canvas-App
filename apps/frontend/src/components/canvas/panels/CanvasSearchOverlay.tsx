...
export default function CanvasSearchOverlay({ onClose, onResults, onFocusNode }: CanvasSearchOverlayProps) {
  ...
  const matches = useMemo((): SearchMatch[] => {
    ...
    activeCanvas.transcripts.forEach((t: CanvasTranscript) => {
      if (t.title.toLowerCase().includes(q) || t.content.toLowerCase().includes(q)) {
        ...
        const context =
          idx >= 0 ? '...' + t.content.slice(Math.max(0, idx - 20), idx + query.length + 30) + '...' : undefined;
  ...
  const totalNodes =
    (activeCanvas?.transcripts.length ?? 0) + (activeCanvas?.questions.length ?? 0) + (activeCanvas?.memos.length ?? 0);
  ...
  return (
    <div className="absolute top-3 left-1/2 z-40 -translate-x-1/2">
      <div className="command-palette-enter w-80 rounded-xl border border-gray-200/80 bg-white/95 shadow-lg backdrop-blur-md dark:border-gray-700/80 dark:bg-gray-800/95 overflow-hidden">
          ...
          <div className="max-h-60 overflow-y-auto p-1">
            {matches.slice(0, 20).map((m, i) => (
              <button
                ...
                key={`${m.nodeId}-${i}`}
                onClick={() => {
                  onFocusNode?.(m.nodeId);
                  onClose();
                }}
                className="flex w-full items-center gap-2 rounded-lg px-2.5 py-1.5 text-left text-xs hover:bg-gray-50 dark:hover:bg-gray-750 transition-colors"
              >
                {m.type === 'code' && m.color ? (
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {m.context && <p className="text-[10px] text-gray-400 dark:text-gray-500 truncate">{m.context}</p>}
                </div>
        ...
        {query.trim() && matches.length === 0 && (
          <div className="py-4 text-center text-xs text-gray-400 dark:text-gray-500">No matches found</div>
        )}
      </div>
    </div>
  );
