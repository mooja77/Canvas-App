  ...
  return (
    ...
    >
      <div className="max-h-[300px] overflow-auto px-2 py-2">
        {!result?.rows?.length ? (
          <p className="text-xs text-gray-400 text-center py-4">Assign transcripts to cases, then click Run.</p>
        ) : (
          <table className="w-full text-[10px]">
            <thead>
              <tr>
                <th className="text-left p-1 text-gray-500 dark:text-gray-400 sticky left-0 bg-white dark:bg-gray-800">
                  Case
                </th>
              ...
              {result.rows.map((row) => (
                <tr key={row.caseId} className="border-t border-gray-100 dark:border-gray-700">
                  ...
                  {row.cells.map((cell) => (
                    <td
                      key={cell.questionId}
                      className="p-1 text-center text-gray-600 dark:text-gray-400"
                      title={cell.excerpts.join('; ')}
                    >
                      {cell.count > 0 ? (
