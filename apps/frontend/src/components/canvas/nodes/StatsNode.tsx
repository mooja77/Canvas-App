...
import type { CanvasComputedNode, StatsConfig, StatsResult } from '@qualcanvas/shared';
...
function StatsNode({ data, id, selected }: NodeProps) {
  ...
  const handleSaveConfig = async () => {
    try {
      await updateComputedNode(node.id, { config: { ...config, groupBy } });
      setEditing(false);
    } catch (err) {
  ...
  return (
    <ComputedNodeShell
      ...
      color="#3B82F6"
      onConfigure={() => {
        setGroupBy((config?.groupBy as 'question' | 'transcript') || 'question');
        setEditing(true);
      }}
      selected={selected}
