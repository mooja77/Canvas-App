...
function ClusterNode({ data, id, selected }: NodeProps) {
  ...
  const handleSaveConfig = async () => {
    try {
      await updateComputedNode(node.id, { config: { ...config, k } });
      setEditing(false);
    } catch (err) {
          ...
          <div className="space-y-3">
            {result.clusters.map((cluster, i) => (
              <div
                ...
                style={{ borderColor: `${CLUSTER_COLORS[i % CLUSTER_COLORS.length]}40` }}
              >
                  ...
                  <div className="flex flex-wrap gap-1 mb-1.5">
                    {cluster.keywords.map((kw) => (
                      <span
                        ...
                        style={{
                          backgroundColor: `${CLUSTER_COLORS[i % CLUSTER_COLORS.length]}20`,
