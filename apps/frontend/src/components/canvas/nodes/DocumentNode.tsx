...
function DocumentNode({ data, selected }: NodeProps) {
  ...
  if (zoomTier === 'minimal') {
    return (
      <div
        className={`px-2 py-1 rounded text-xs font-medium truncate max-w-[120px] border ${
    ...
    >
      ...
      >
        <div className="flex items-center gap-2 min-w-0">
          <span className="text-sm">{doc.docType === 'pdf' ? '\uD83D\uDCC4' : '\uD83D\uDDBC\uFE0F'}</span>
          <h3 className="text-sm font-semibold text-gray-800 dark:text-gray-200 truncate">{doc.title}</h3>
        </div>
      ...
      {!collapsed && (
        <div className="p-3">
          ...
          <div className="relative bg-gray-100 dark:bg-gray-700 rounded border border-gray-200 dark:border-gray-600 min-h-[200px] flex items-center justify-center">
            <span className="text-gray-400 dark:text-gray-500 text-sm">Document Preview</span>
