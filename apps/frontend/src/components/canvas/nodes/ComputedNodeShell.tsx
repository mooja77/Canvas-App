...
function ComputedNodeShell({
  ...
  return (
    <div
      className={`min-w-[280px] w-full h-full rounded-xl border-2 shadow-node transition-all duration-200 hover:shadow-node-hover ${selected ? 'ring-2 ring-blue-400' : ''} ${running || isRunningGlobal ? 'animate-pulse ring-2 ring-offset-1' : ''}`}
      style={{
        borderColor: running || isRunningGlobal ? color : color,
        boxShadow: running || isRunningGlobal ? `0 0 12px ${color}40` : undefined,
        ...(running || isRunningGlobal ? ({ '--tw-ring-color': `${color}99` } as React.CSSProperties) : {}),
      }}
    >
      <NodeResizer
        minWidth={280}
