...
function ComputedNodeShell({
  ...
  return (
    <div
      className={`group relative min-w-[280px] w-full h-full rounded-xl bg-white/95 dark:bg-gray-800/95 backdrop-blur-sm shadow-node transition-all duration-200 hover:shadow-node-hover hover:-translate-y-px ${
        selected || running || isRunningGlobal ? 'ring-2' : 'ring-1 ring-gray-200/70 dark:ring-gray-700/60'
      } ${running || isRunningGlobal ? 'animate-pulse' : ''}`}
      style={{
        ...(selected || running || isRunningGlobal
          ? ({ '--tw-ring-color': running || isRunningGlobal ? `${color}99` : color } as React.CSSProperties)
          : {}),
        ...(running || isRunningGlobal ? { boxShadow: `0 0 16px ${color}40` } : {}),
      }}
    >
      {/* Color identity spine — a calm left-edge accent matching the Code node,
          so every analysis type reads its identity at a glance (inset + rounded
          so it sits cleanly inside the card corners without overflow clipping). */}
      <div
      ...
      {/* Header — calm and transparent over the card surface; the tinted icon +
          color spine carry identity. Run stays present (it's the node's whole
          purpose); secondary controls reveal on hover so the result is the hero. */}
      ...
      {/* Body - collapsible (transparent so the card reads as one elevated surface) */}
      {!collapsed && !isZoomedOut && (
        <div className="relative z-10 rounded-b-xl nodrag nowheel overflow-hidden">
          {error && (
