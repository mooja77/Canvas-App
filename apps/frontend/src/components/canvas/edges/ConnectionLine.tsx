...
export default function ConnectionLine({ fromX, fromY, toX, toY, fromNode }: ConnectionLineComponentProps) {
  ...
  return (
    <g>
      ...
      {/* SMIL <animate> escapes the CSS reduced-motion reset, so gate it explicitly. */}
      <circle cx={toX} cy={toY} r={4} fill={color} opacity={0.8}>
        {!reducedMotion && <animate attributeName="r" values="3;5;3" dur="1s" repeatCount="indefinite" />}
      </circle>
    </g>
  );
}
