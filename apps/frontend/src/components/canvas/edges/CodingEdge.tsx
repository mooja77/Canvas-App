...
export default function CodingEdge({
  ...
  const [deletingCodingIds, setDeletingCodingIds] = useState<string[]>([]);
  ...
  return (
    <>
          ...
          >
            <div className="edge-tooltip-enter max-w-[260px] rounded-xl bg-gray-900/95 px-3 py-2.5 text-xs text-white shadow-lg backdrop-blur-md ring-1 ring-white/10">
              ...
              {count > 1 && codings ? (
                <div className="max-h-[150px] overflow-y-auto space-y-1.5 pr-1 scrollbar-thin scrollbar-thumb-gray-700">
                  {codings.map((c) => (
                    <div key={c.id} className="group flex items-start gap-1.5">
                      <p className="flex-1 line-clamp-2 leading-relaxed text-gray-200">&ldquo;{c.codedText}&rdquo;</p>
                      <button
                        onClick={() => {
                          setDeletingCodingIds([c.id]);
                ...
                <p className="line-clamp-3 leading-relaxed text-gray-200">&ldquo;{edgeData.codedText}&rdquo;</p>
              )}

              <button
                onClick={() => {
                  if (count === 1 && edgeData.codingId) {
                    setDeletingCodingIds([edgeData.codingId]);
                  } else if (codings && codings.length > 0) {
                    setDeletingCodingIds(codings.map((coding) => coding.id));
                  }
                  setShowDeleteConfirm(true);
                }}
      ...
      {/* Delete confirmation */}
      {showDeleteConfirm &&
        deletingCodingIds.length > 0 &&
        createPortal(
          <ConfirmDialog
            title="Remove Coding"
            message={
              deletingCodingIds.length > 1
                ? 'Remove all coded segments on this edge? The transcript text will not be affected.'
                : 'Remove this coded segment? The transcript text will not be affected.'
            }
            confirmLabel="Remove"
            onConfirm={async () => {
              await Promise.all(deletingCodingIds.map((codingId) => deleteCoding(codingId)));
              setShowDeleteConfirm(false);
              setDeletingCodingIds([]);
              setHovered(false);
            }}
            onCancel={() => {
              setShowDeleteConfirm(false);
