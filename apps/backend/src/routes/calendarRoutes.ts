...
const createCalendarEventSchema = z.object({
  ...
  endDate: z.string().refine(s => !isNaN(Date.parse(s)), 'Invalid end date').optional(),
...
const updateCalendarEventSchema = z.object({
  ...
  startDate: z.string().refine(s => !isNaN(Date.parse(s)), 'Invalid start date').optional(),
  endDate: z.string().refine(s => !isNaN(Date.parse(s)), 'Invalid end date').optional().nullable(),
...
calendarRoutes.post('/calendar/events', validate(createCalendarEventSchema), async (req, res, next) => {
  try {
    const userId = requireUser(req);
...
calendarRoutes.put('/calendar/events/:id', validateParams(eventIdParam), validate(updateCalendarEventSchema), async (req, res, next) => {
  try {
    const userId = requireUser(req);
    const { id } = req.params;
...
calendarRoutes.delete('/calendar/events/:id', validateParams(eventIdParam), async (req, res, next) => {
  try {
    const userId = requireUser(req);
    const { id } = req.params;
