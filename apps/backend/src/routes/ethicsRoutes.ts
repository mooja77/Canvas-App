...
ethicsRoutes.put(
  '/canvas/:canvasId/ethics',
  validate(updateEthicsSchema),
  checkEthicsAccess(),
  async (req, res, next) => {
    try {
      const dashboardAccessId = getAuthId(req);
  ...
  async (req, res, next) => {
    try {
      ...
      logAudit({
        ...
        method: 'POST',
        path: req.originalUrl,
        // Hash participantId so the audit trail can still correlate events
        // for the same participant without storing the raw identifier.
        meta: JSON.stringify({
          canvasId: req.params.canvasId,
          participantIdHash: sha256(participantId),
...
ethicsRoutes.put(
  '/canvas/:canvasId/consent/:consentId/withdraw',
  ...
  async (req, res, next) => {
    try {
      ...
      logAudit({
        ...
        method: 'PUT',
        path: req.originalUrl,
        meta: JSON.stringify({ canvasId: req.params.canvasId, participantIdHash: sha256(updated.participantId) }),
      });
...
ethicsRoutes.post(
  '/canvas/:canvasId/transcripts/:transcriptId/anonymize',
  validate(anonymizeTranscriptSchema),
  checkEthicsAccess(),
  async (req, res, next) => {
    try {
      const dashboardAccessId = getAuthId(req);
