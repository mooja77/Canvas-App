...
async function openPerformanceCanvas(page: any) {
...
test.describe('Deep Canvas: Performance Under Load', () => {
  test.beforeAll(async ({ browser }) => {
    ...
    const longText = Array(60)
      .fill(
        'The researcher noted that participants described complex and multifaceted experiences navigating the healthcare system.',
      )
      .join(' ');
    const tRes = await p.request.post(`${API}/canvas/${canvasId}/transcripts`, {
      headers: headers(),
      data: { title: 'Large Transcript', content: longText },
    });
    ...
    for (let i = 0; i < 30; i++) {
      ...
      await p.request.post(`${API}/canvas/${canvasId}/codings`, {
        ...
        data: {
          ...
          questionId: codeId,
          startOffset: start,
          endOffset: Math.min(end, longText.length - 1),
          codedText: longText.substring(start, Math.min(end, longText.length - 1)),
        },
      });
    }
    await p.close();
  ...
  test('1 - Canvas loads with 15 codes in navigator', async ({ page }) => {
    await openPerformanceCanvas(page);
